...
# endif
   ...
   if (chars_buffered_log > 0)
   {
      ...
      /*
       * So that we do not read the logs at AWSD_LOG_CHECK_INTERVAL time
       ...
       * data at MAX_LOG_DATA_BUFFER / AWSD_LOG_CHECK_INTERVAL bytes
       * per second.
       */
      ...
      {
         log_interval = AWSD_LOG_CHECK_INTERVAL;
      }
   }
   else
   {
